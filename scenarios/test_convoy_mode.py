import sys
import os
import time
import random
import threading
import psutil

# Allow import from root dir
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from config import LOG_FOLDER, SIMULATION_STEPS
from environment import SimulationEnvironment
from car import Car
from satellite import Satellite
from logger import Logger
from region_manager import RegionManager

def profile(func):
    def wrapper(*args, **kwargs):
        start = time.time()
        result = func(*args, **kwargs)
        print(f"[Profiler] {func.__name__} took {time.time() - start:.4f}s")
        return result
    return wrapper

def monitor_resources():
    process = psutil.Process(os.getpid())
    cpu = psutil.cpu_percent(interval=None)
    mem = process.memory_info().rss / (1024 * 1024)
    print(f"[Resource Monitor] CPU: {cpu:.2f}%, Memory: {mem:.2f} MB")

@profile
def run_step(step, cars, env, logger, satellite, region_manager, convoy_groups, global_hazard_memory):
    env.step()

    def detect_and_log(car):
        hazards = car.detect_hazards()
        for hx, hy in hazards:
            key = (hx, hy)
            global_hazard_memory[key] = global_hazard_memory.get(key, []) + [car.car_id]
            car.intelligence_score += 1
            logger.record_detection(step, car.car_id, hx, hy)

    threads = []
    for car in cars:
        thread = threading.Thread(target=detect_and_log, args=(car,))
        threads.append(thread)
        thread.start()
    for t in threads:
        t.join()

    # Share info within convoy groups first
    for convoy in convoy_groups:
        for car in convoy:
            for (hx, hy), sharers in global_hazard_memory.items():
                if car.car_id not in sharers and any(s in [c.car_id for c in convoy] for s in sharers):
                    if random.random() < 0.95:  # almost guaranteed inside convoy
                        car.receive_hazard(hx, hy)
                        logger.record_received(step, car.car_id, hx, hy, sender_id=None)

    # Satellite override
    for car in cars:
        satellite_data = satellite.scan_for_hazards(car)
        for hx, hy, confidence in satellite_data:
            if confidence > 0.7:
                car.receive_hazard(hx, hy)
                car.intelligence_score += int(confidence * 10)
                logger.record_satellite_info(step, car.car_id, hx, hy, confidence)

def main():
    env = SimulationEnvironment()
    satellite = Satellite(env)
    region_manager = RegionManager()
    logger = Logger()

    cars = [Car(i, env, logger, region_manager, satellite) for i in range(env.num_cars)]
    convoy_groups = [cars[i:i + 5] for i in range(0, len(cars), 5)]  # groups of 5 cars

    for car in cars:
        car.intelligence_score = 0

    hazard_memory = {}

    for step in range(SIMULATION_STEPS):
        run_step(step, cars, env, logger, satellite, region_manager, convoy_groups, hazard_memory)
        monitor_resources()

    logger.save(cars)

if __name__ == "__main__":
    main()
    from visualizer import visualize_results
    visualize_results()